...
namespace RayTween.Plugins
{
    ...
    { 
        ...
        public UnsafeString Evaluate(ref UnsafeString startValue, ref UnsafeString endValue, float progress)
        {
           ...
           if (!Buffer.IsCreated)
           {
               Buffer.Value= new UnsafeList<ushort>(math.max(startValue.Length,endValue.Length), Allocator.Persistent);
           }
           Buffer.Value.Clear();
           StringHelper.Interpolate(ref Buffer.Value,ref startValue.Value, ref endValue.Value, progress, ScrambleMode, RichTextEnabled, ref RandomState, ref CustomScrambleChars.Value);
           return Buffer;
        }
